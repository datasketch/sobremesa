window.addEventListener("DOMContentLoaded", function () {
  try {
    ...
    const buttonToolsUnactive = document.getElementById(
      "button-tools-unactive"
    );

    const recommendedToolsPreview = document.getElementById(
      "recommended-tools-preview"
    );
    const recommendedToolsViewMore = document.getElementById(
      "recommended-tools-view-more"
    );
    const buttonRecommendedToolsActive = document.getElementById(
      "button-recommended-tools-active"
    );
    const buttonRecommendedToolsUnactive = document.getElementById(
      "button-recommended-tools-unactive"
    );

    buttonToolsActive &&
      buttonToolsActive.addEventListener("click", () => {
    ...
    buttonToolsUnactive &&
      buttonToolsUnactive.addEventListener("click", () => {
    ...
    buttonRecommendedToolsActive &&
      buttonRecommendedToolsActive.addEventListener("click", () => {
        ...
        document
          .getElementById("scroll-to-2")
          .scrollIntoView({ behavior: "smooth", block: "start" });
      });

    buttonRecommendedToolsUnactive &&
      buttonRecommendedToolsUnactive.addEventListener("click", () => {
    ...
    function validateFilters() {
      ...
      cards.forEach((card) => {
        ...
        const hasSearch =
          removeAccents(card.dataset.title).includes(removeAccents(search)) ||
          removeAccents(card.dataset.description).includes(
    ...
    forWhomEl &&
      forWhomEl.addEventListener("input", () => {
        validateFilters();
      });
